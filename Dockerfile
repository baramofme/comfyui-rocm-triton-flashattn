...
# skips the HIP C++ extension (flash_attn_2_cuda, CDNA-only gfx90a/gfx942) and
# builds only the Triton backend — the only path that works on RDNA3 (gfx1100).
# No aiter subprocess, no libtriton.so overwrite, no save/restore needed.
